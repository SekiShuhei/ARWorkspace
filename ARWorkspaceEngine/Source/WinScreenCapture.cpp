...
DWORD WinScreenCapture::GetBitmapImageSize(const BITMAPINFO& bitmap_info) const
{
	...
	case 4:
	{
		DWORD pixelsPerDW = 8 * sizeof(DWORD) / bitmap_info.bmiHeader.biBitCount; // DWORD ì‡ÇÃâÊëfêî
	...
	case 32:
	{
		DWORD bytesPerPixel = bitmap_info.bmiHeader.biBitCount / 8; // ÇPâÊëfìñÇΩÇËÇÃÉoÉCÉgêî
...
bool WinScreenCapture::CaptureScreen(s3d::Image& read_image, int x, int y, int width, int height)
{
	...
	if (hPrevBitmap != NULL)
	{
		::DeleteObject(hPrevBitmap);  //BMPÇçÌèúÇµÇΩéûÅAlpPixelÇ‡é©ìÆìIÇ…âï˙Ç≥ÇÍÇÈ
	}
	if (hPrevMemDC != NULL)
	{
		::DeleteDC(hPrevMemDC);
